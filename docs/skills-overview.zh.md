...
- 主要流程：检测 `git-dir` 与 `git-common-dir` 判断是否已在 linked worktree，并排除 submodule；优先消费 Unified Execution Handoff 或已有用户指令中的 workspace 决策，不重复询问。手动 fallback 默认使用项目内 `.worktrees/`；若缺少 ignore rule，project-local worktree consent 允许最小追加一次 `.gitignore`，但不授权 commit。显式 global preference 不编辑仓库；之后执行 setup 和 baseline verification。
- 关键约束：不要嵌套创建 worktree；不要在有原生工具时手写 `git worktree add`；不要重复 ignore rule；不要把 setup edit 误作 commit authorization；不要跳过 baseline verification。
