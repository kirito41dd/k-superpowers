...
- `test-driven-development`、`systematic-debugging`、`verification-before-completion` 是质量纪律，负责阻止猜测、补测、未验证宣称等常见失败模式。
...
| `test-driven-development` | `Use when implementing any feature or bugfix, before writing implementation code` | 实现功能或 bugfix 前触发。注入重点是“写实现代码之前”，触发清晰但不展开 Red-Green-Refactor。 |
...
- 主要流程：Phase 1 读完整错误、稳定复现、检查近期变更、在多组件边界加诊断、追踪数据流；Phase 2 找工作示例、读参考实现、比较差异、理解依赖；Phase 3 写出单一假设并用最小变化验证；Phase 4 创建失败测试、实现单一根因修复、验证修复和回归。
- 关键约束：不能“先试一个”；不能多改一起跑；修了 3 次还失败要停下来质疑架构，而不是继续补丁。

### test-driven-development

- 触发场景：实现任何功能、bugfix、重构或行为变更之前。
- 核心思想：先写测试、看它按预期失败、再写最小代码通过；没看过失败就不知道测试是否有效。
- 主要流程：RED 写一个最小行为测试；运行并确认失败原因正确；GREEN 写最少代码让它通过；运行并确认通过且输出干净；REFACTOR 只在绿色状态下清理命名、重复和结构；重复下一行为。
- 关键约束：先写了生产代码就删除重来；测试后补不是 TDD；测试通过得太早说明测的是已有行为；不要以手工验证替代自动测试。
...
- 主要流程：先检查 spec 是否过大或跨多个独立子系统；规划文件结构和职责边界；把任务拆成 2-5 分钟粒度的步骤；每个任务列出精确文件、测试、代码片段、命令和预期输出；要求 TDD、频繁 commit；写完后自检 spec 覆盖、占位符、类型和命名一致性；保存到 `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`；让用户选择 subagent-driven 或 inline execution。
