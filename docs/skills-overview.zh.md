...
- `test-driven-development` 在本 fork 中保留历史名称，但语义改为类型优先验证；`systematic-debugging`、`verification-before-completion` 继续负责阻止猜测和未验证宣称。
- `requesting-code-review`、`receiving-code-review` 和 subagent review 流程把评审变成强制质量门。
- `writing-skills` 把 skill 本身视为会影响 agent 行为的“代码”，要求用类似 TDD 的方式测试和迭代。
...
3. `writing-plans`：把设计拆成可执行、可验证、可交给低上下文 agent 的计划。
...
| `test-driven-development` | `Use when implementing behavior that needs explicit verification, especially core logic, bug fixes, public APIs, algorithms, parsers, protocols, state machines, or high-risk changes` | 本 fork 保留 skill 名但改为类型优先验证。触发条件收窄到需要显式验证的行为，避免所有改动都进入严格 TDD。 |
...
- 主要流程：先探索项目上下文；如果涉及视觉问题，单独询问是否启用 visual companion；一次只问一个澄清问题；提出 2-3 个方案与权衡；分段展示设计并逐段确认；写入 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`；自检占位符、矛盾、范围和歧义；要求用户 review spec；批准后转入 `writing-plans`。
...
- 核心思想：早评审、常评审，用隔离上下文的 reviewer agent 在问题扩散前发现缺陷。
- 主要流程：确定 base/head SHA；用 `requesting-code-review/code-reviewer.md` 模板派发 reviewer；提供变更说明、计划/需求、SHA 范围；处理反馈：Critical 立即修、Important 继续前修、Minor 可记录；若 reviewer 错误则用证据反驳。
- 关键约束：不要因为“很简单”跳过 review；不要忽略 Critical/Important；不要让 reviewer 继承当前会话历史，必须提供精确上下文。

### subagent-driven-development

- 触发场景：已有实现计划，任务基本独立，并希望在当前会话中连续执行。
- 核心思想：每个任务使用新鲜 implementer subagent，随后做两阶段评审：先 spec compliance，再 code quality。
- 主要流程：读取计划并提取所有任务全文；建立 todo；每个任务派发 implementer；处理 implementer 的 `DONE`、`DONE_WITH_CONCERNS`、`NEEDS_CONTEXT`、`BLOCKED` 状态；通过 spec reviewer 确认没有少做或多做；通过 code quality reviewer 检查实现质量；有问题回到 implementer 修复并复审；所有任务完成后做最终 review，再转 `finishing-a-development-branch`。
- 关键约束：不要并行派发多个实现 agent；不要让 implementer 自己读计划文件；spec 未通过前不能做 code quality review；两个 review 任一有开放问题都不能进入下一任务。

### systematic-debugging

- 触发场景：任何 bug、测试失败、构建失败、性能问题或非预期行为。
- 核心思想：没有根因调查就不能修复；症状修复就是失败。
- 主要流程：Phase 1 读完整错误、稳定复现、检查近期变更、在多组件边界加诊断、追踪数据流；Phase 2 找工作示例、读参考实现、比较差异、理解依赖；Phase 3 写出单一假设并用最小变化验证；Phase 4 先选择验证方式，再实现单一根因修复并验证。
- 关键约束：不能“先试一个”；不能多改一起跑；修了 3 次还失败要停下来质疑架构，而不是继续补丁。

### test-driven-development

- 触发场景：实现需要显式验证的行为，例如核心逻辑、bugfix、公共 API、算法、parser、协议、状态机或高风险变更。
- 核心思想：优先用类型、接口、可见性和编译器表达不变量；测试只覆盖类型无法证明或回归代价高的行为。
- 主要流程：明确行为/不变量；先设计类型和 API 边界；识别编译器无法证明的风险；只为核心行为和回归风险加聚焦测试；实现最小变更；运行相关验证。
- 关键约束：不再要求所有改动严格 test-first；不能以“有类型”逃避运行时行为验证；测试应保护核心语义而非锁死实现细节。
...
- 主要流程：先检查 spec 是否过大或跨多个独立子系统；规划文件结构和职责边界；把任务拆成 2-5 分钟粒度的步骤；每个任务列出精确文件、类型/API 边界、必要测试、代码片段、验证命令和预期输出；要求类型优先、聚焦验证、频繁 commit；写完后自检 spec 覆盖、占位符、类型和命名一致性；保存到 `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`；让用户选择 subagent-driven 或 inline execution。
- 关键约束：不能写 “TODO/TBD/类似上一步/添加适当错误处理” 这类占位；代码步骤必须给实际代码；命令必须给预期结果。

### writing-skills

- 触发场景：创建、修改或验证 skill。
- 核心思想：写 skill 是把 TDD 应用于流程文档；skill 会塑造 agent 行为，因此必须先看 agent 在没有 skill 时如何失败。
- 主要流程：RED 阶段构造压力场景并在没有 skill 时运行，记录失败和 rationalization；GREEN 阶段写最小 skill，覆盖已观察到的失败；再次运行压力场景确认 agent 遵守；REFACTOR 阶段找新漏洞、补显式反制、复测直到稳定。
- 关键约束：没有 failing test 就不能写或改 skill；description 只描述触发条件，不能概括流程；不要批量创建多个未验证 skill；行为塑造内容变更需要 adversarial eval。

## 后续微调建议

- 先区分“个人偏好”与“可贡献上游”：个人偏好放 fork 文档、项目记忆或独立插件；不要直接改 core skill 正文后开 PR。
- 改 behavior-shaping 内容前，先用 `writing-skills` 做压力场景，记录 before/after，否则很容易被 upstream 拒绝。
