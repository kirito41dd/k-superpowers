...
- `type-driven-verification` 负责类型优先验证；`systematic-debugging`、`verification-before-completion` 继续负责阻止猜测和未验证宣称。
...
- 触发场景：创建、修改或验证 skill。
- 核心思想：skill 会塑造 agent 行为，因此要先明确行为不变量、触发条件和风险，再按风险选择静态审查、文本搜索、压力场景或 before/after eval。
- 主要流程：低风险措辞或流程 gate 变更先写清不变量并做最小修改，再用搜索和反例检查确认没有冲突；高风险行为塑造、触发条件或 subagent 流程变更再构造压力场景并做 before/after eval。
- 关键约束：description 只描述触发条件，不能概括流程；不要批量创建多个未验证 skill；验证强度应匹配行为风险，不默认套 RED/GREEN。
