...
3. `writing-plans`：把设计拆成可执行、可验证、可交给低上下文 agent 的计划，默认偏向可独立验证的 vertical slice。
...
- 主要流程：读取计划并提取所有任务全文；建立 todo；每个任务派发 implementer；处理 implementer 的 `DONE`、`DONE_WITH_CONCERNS`、`NEEDS_CONTEXT`、`BLOCKED` 状态；通过 spec reviewer 确认没有少做或多做；通过 code quality reviewer 检查实现质量、行为边界和项目约定；有问题回到 implementer 修复并复审；所有任务完成后做最终 review，再转 `finishing-a-development-branch`。
...
- 主要流程：先检查 spec 是否过大或跨多个独立子系统；规划文件结构和职责边界；默认把任务拆成可独立验证的 vertical slice，每个任务写清 slice 行为、依赖、文件、验证命令；任务模板按类型优先顺序展开：定义类型/API 边界（显式写出类型证不了的 Runtime risk）→ 最小实现 → 只对类型证不了的行为加聚焦测试 → 运行验证 → 授权时才做 commit checkpoint；bug 修复任务的回归测试需先复现失败再修复；每个任务列出精确文件、代码片段、验证命令和预期输出；写完后自检 spec 覆盖、占位符、类型和命名一致性；保存到 `docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md`；让用户选择 subagent-driven 或 inline execution。
- 关键约束：不能写 “TODO/TBD/类似上一步/添加适当错误处理” 这类占位；代码步骤必须给实际代码；命令必须给预期结果；按层/按文件拆任务只适用于真实前置类型/API 边界、必要 prefactor、机械迁移或文档/config 变更，并且要有清晰验证；commit 政策由 Execution Handoff 下的 `Commit Authorization` 小节单一承载——plan 文档只在用户显式选择时提交，批准计划不等于授权实现或提交实现代码。
