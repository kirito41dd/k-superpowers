...
- 主要流程：先检查 spec 是否过大或跨多个独立子系统；规划文件结构和职责边界；把项目级要求复制到 `Global Constraints`；默认拆成可独立验证且值得单独 review 的 vertical slice；每个任务写清 slice、依赖、文件、`Interfaces`、验证命令，并按行为影响标记 `Risk: low | medium | high` 与具体 rationale，识别共享接口/状态和跨 Task 组合行为。任务模板按类型优先顺序展开：定义类型/API 边界与 Runtime risk → 最小实现及必要核心说明 → 聚焦测试 → 验证 → 授权时 commit；写完后自检 spec 覆盖、占位符、类型/接口/约束一致性、核心说明、任务大小和风险分级；保存计划并让用户选择风险自适应 SDD 或 inline execution。SDD 选择后仍要单独取得 local checkpoint commit 授权。
