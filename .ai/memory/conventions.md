...
<a id="conv-20260721-01"></a>

- **状态**：有效
- **范围**：`AGENTS.md`, `skills/*`, `docs/skills-overview.zh.md`, `README.md`。
- **来源**：迁移前记录：本仓库 commit `94bb0264f83a54d6b5f31af25ebdf433d14f7fc5` 中 `.ai/memory/conventions.md` 的同名条目；原始对话链接未附。
- **标签**：#skills #judgment-first
...
<a id="conv-20260721-02"></a>

- **状态**：有效
- **范围**：`skills/*`, `README.md`, `docs/skills-overview.zh.md`。
- **来源**：迁移前记录：本仓库 commit `94bb0264f83a54d6b5f31af25ebdf433d14f7fc5` 中 `.ai/memory/conventions.md` 的同名条目；原始对话链接未附。
- **标签**：#skills #iteration #verification
- **替代（supersedes）**：[2026-05-21 Skill 正文变更必须先评估](conventions.md#conv-20260521-04)
...
- **来源**：迁移前记录：本仓库 commit `94bb0264f83a54d6b5f31af25ebdf433d14f7fc5` 中 `.ai/memory/conventions.md` 的同名条目；原始对话链接未附。
- **标签**：#skills #routing #comments

- **约定**：普通知识问答、闲聊和无需仓库/工具/workflow 的解释应直接回答，入口 bootstrap 不等于必须调用 task skill，也不得输出 skill 公告或流程 ceremony。只有当前阶段确有 owner 时才加载最小充分 skill。任何 prompt/skill 瘦身都必须保留 [2026-07-15 核心代码说明由 type-driven-verification 单一拥有](conventions.md#conv-20260715-01) 的传播链：核心解释要求、项目/邻近文件决定的注释形式与语言、self-explanatory 排除，以及 implementer/reviewer 的独立自包含检查。
...
- **约定**：核心结构、函数和抽象的解释性注释/文档原则由 `skills/type-driven-verification/SKILL.md` 完整拥有；`writing-plans` 和 generic code reviewer 只引用该 owner，避免在高频 skill 中复制整套规则。SDD implementer/reviewer prompt 因子 Agent 上下文独立，可保留直接且自包含的要求。完整内容原则继续以 [2026-07-07 Skill 写代码时的注释原则](conventions.md#conv-20260707-01) 为准。
...
- **来源**：迁移前记录：本仓库 commit `94bb0264f83a54d6b5f31af25ebdf433d14f7fc5` 中 `.ai/memory/conventions.md` 的同名条目；原始对话链接未附。
- **标签**：#opencode #install
- **被替代为**：[2026-06-10 三条安装链路并列维护，CC 插件命名 k-superpowers](conventions.md#conv-20260610-01)
- **核验记录**：2026-09-08：依据 [2026-06-10 三条安装链路并列维护，CC 插件命名 k-superpowers](conventions.md#conv-20260610-01) 补齐替代关系。安装链路改为三条并列维护。历史正文与实施结果保留。

> 历史变更说明（2026-06-10）： 已被「三条安装链路并列维护，CC 插件命名 k-superpowers」取代：CC/Codex 本地 marketplace 与 OpenCode 并列，不再以 OpenCode 为优先。
...
<a id="conv-20260521-04"></a>

- **状态**：已替代
- **范围**：`skills/*/SKILL.md`, `skills/*/*.md` 中会影响 agent 行为的内容。
- **来源**：迁移前记录：本仓库 commit `94bb0264f83a54d6b5f31af25ebdf433d14f7fc5` 中 `.ai/memory/conventions.md` 的同名条目；原始对话链接未附。
- **标签**：#skills #verification #eval
- **被替代为**：[2026-07-21 Skill 修改采用一次编辑一次自审](conventions.md#conv-20260721-02)
- **核验记录**：2026-09-08：依据 [2026-07-21 Skill 修改采用一次编辑一次自审](conventions.md#conv-20260721-02) 补齐替代关系。默认压力场景和 before/after eval 被取消，行为边界与静态自审由新约定承接。历史正文与实施结果保留。

> 历史变更说明（2026-07-21）： 其中压力场景和 before/after eval 要求已被「Skill 修改采用一次编辑一次自审」取代；先明确行为边界、做静态审查和反例检查的原则继续有效。
...
<a id="conv-20260521-05"></a>

- **状态**：已替代
- **范围**：`.ai/memory/*`, `CLAUDE.md`
- **来源**：迁移前记录：本仓库 commit `94bb0264f83a54d6b5f31af25ebdf433d14f7fc5` 中 `.ai/memory/conventions.md` 的同名条目；原始对话链接未附。
- **标签**：#memory
- **被替代为**：[2026-09-08 升级项目记忆的检索、授权和维护规则](decisions.md#dec-20260908-01)
- **核验记录**：2026-09-08：依据 [2026-09-08 升级项目记忆的检索、授权和维护规则](decisions.md#dec-20260908-01) 补齐替代关系。渐进式披露继续保留；检索复用、稳定 ID、写入授权和归档边界统一由 policy.md 定义。历史正文与实施结果保留。
