...
- `using-superpowers`：普通问答直接回答；清晰、已批准的变更走 Direct，安全默认是 current workspace + Inline + no commit，不再强制 spec/plan/handoff ceremony。
- `brainstorming` / `writing-plans`：只有真实取舍才列方案，只有交接价值才落持久 spec/plan；Full 保护 material decision，而不是逐章节审批。
- `type-driven-verification`：采用类型优先、风险驱动验证，并单一拥有核心代码说明合同。设计问题是按需思考维度，不是必填表单；注释语言跟随项目和邻近文件。
- `subagent-driven-development`：只在独立任务且委派收益明确时使用。Low 由 controller 处理，medium 为 implementer + controller 自审，high 才派 reviewer；final review 只保护真实跨任务 integration risk。
- `requesting-code-review`：一次 Discovery、一次批量修复、一次 Closure。Stable finding 信息保留，但不冻结输出编码或 tool 顺序；reviewer 可为具体问题读取必要上下文。
...
```text
k-superpowers@k-superpowers-dev  installed, enabled  5.4.1
```
